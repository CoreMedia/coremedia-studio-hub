...
public interface ConnectorContentService {
   ...
   * @param connectorId the connector id that is stored in the created content, if exists
   * @param site the site used as search filter for the content or null for a global lookup
   */
  @Nullable
  Content findContent(@Nonnull ConnectorId connectorId, @Nullable Site site);
   ...
   * @param item the item that contains the external system data
   * @param wait true to wait until the post processing is finished.
   */
  void processContent(@Nonnull Content content, @Nonnull ConnectorItem item, boolean wait);
