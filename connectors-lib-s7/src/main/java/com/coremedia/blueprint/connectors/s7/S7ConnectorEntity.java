...
/*
  ...
  public String getName() {
    if (file.getAsset() != null) {
      switch (file.getAsset().getType().toLowerCase()) {
        case "image":
          return file.getAsset().getFileName();
        case "video":
          return file.getAsset().getFileName();
        default:
          return name;
  ...
  public String getUrl() {
    if (file.getAsset() != null) {
      switch (file.getAsset().getType().toLowerCase()) {
        case "image":
          return String.format(assetUrlPattern, file.getAsset().getIpsImageUrl());
        case "video":
          return String.format(videoUrlPattern, file.getAsset().getIpsImageUrl());
      }
    }

    http:
//s7d2.scene7.com/is/image/CoreMedia/maxresdefault?scl=1
    https:
//s7ips1.scene7.com/is/content/CoreMedia/Best%20Of%20Medien%C3%BCbergreifendes%20Anwendungsprogramm
