...
public class S7ConnectorServiceImpl extends FileBasedConnectorService<S7Container> {
  ...
  public boolean init(@NonNull ConnectorContext context) throws ConnectorException {
    ...
    String password = context.getProperty("password");
       /* try {
           // password = EncryptUtils.decrypt(context.getProperty("password"));
        } catch (GeneralSecurityException | IOException e) {
            throw new ConnectorException("Can't decrypt password");
        }*/
  ...
  public ConnectorItem getItem(@NonNull ConnectorContext connectorContext, @NonNull ConnectorId connectorId) throws ConnectorException {
    ConnectorId parentFolderId = getFolderId(connectorId);
  ...
  public ConnectorCategory getCategory(@NonNull ConnectorContext connectorContext, @NonNull ConnectorId connectorId) throws ConnectorException {
    ...
    S7Container file = getCachedFileOrFolderEntity(connectorContext, connectorId);
  ...
  @Override
  public S7Container getFile(ConnectorId id) {
    if (id.isRootId()) {
      return null;
    }
  ...
  @Override
  public boolean isFile(S7Container entry) {
   ...
   /* @Override
    public Boolean refresh(@NonNull ConnectorCategory category) {
        if (category.getConnectorId().isRootId()) {
            LOGGER.info("Clearing root category");
            rootCategory = (S7ConnectorCategory) getRootCategory();
            itemsMap.remove(ROOT_PATH);
        }
        itemsMap.remove(category.getConnectorId().getExternalId());
        LOGGER.info("Clearing category: {}, size: {}", category.getConnectorId().getExternalId());
  ...
  private List<ConnectorCategory> getSubCategories(@NonNull ConnectorContext connectorContext, @NonNull ConnectorCategory category) throws ConnectorException {
    ...
    List<S7Container> subfolders = getSubfolderEntities(connectorContext, category.getConnectorId());
  ...
  private List<ConnectorItem> getItems(@NonNull ConnectorContext connectorContext, @NonNull ConnectorCategory category) throws ConnectorException {
    ...
    List<S7Container> fileEntities = getFileEntities(connectorContext, category.getConnectorId());
