...
public class ConnectorS7Configuration {
  ...
  @Bean
  @Scope(BeanDefinition.SCOPE_PROTOTYPE)
  public ConnectorService connectorS7Service(@NonNull @Qualifier("s7ConnectorCache") FileSystemService fileSystemService) {
    S7ConnectorServiceImpl s7ConnectorService = new S7ConnectorServiceImpl();
    s7ConnectorService.setFileCache(fileSystemService);
    return s7ConnectorService;
  }

  @Bean
  @Scope(SCOPE_PROTOTYPE)
  public FileSystemService s7ConnectorCache() {
